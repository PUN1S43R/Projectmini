...
async function processTextToFiles(text: string, originalName: string, originalFile: string) {
  ...
  try {
    ...
    const lineHeight = fontSize * 1.1; // Standard line height
    ...
    for (const group of groupedContent) {
      ...
      for (let i = 0; i < allWords.length; i++) {
        ...
        const wordWidth = font.widthOfTextAtSize(sanitizedText, fontSize);
        ...
        if (currentLineWidth + wordWidth + spaceWidth > contentWidth && currentLine.length > 0) {
          ...
          const totalWordsWidth = currentLine.reduce((sum, item) => {
            ...
            return sum + itemFont.widthOfTextAtSize(sText, fontSize);
          }, 0);
          ...
          for (let j = 0; j < currentLine.length; j++) {
            ...
            if (item.isUnderlined) {
              const textWidth = itemFont.widthOfTextAtSize(sText, fontSize);
              page.drawLine({
            ...
            xOffset += itemFont.widthOfTextAtSize(sText, fontSize) + spaceBetween;
          }
      ...
      if (currentLine.length > 0) {
        ...
        for (const item of currentLine) {
          ...
          xOffset += itemFont.widthOfTextAtSize(sText, fontSize) + itemFont.widthOfTextAtSize(" ", fontSize);
        }
        y -= lineHeight + 11; // Restored vertical gap between blocks
